...
2. **执行解锁步骤**
   ...
   - 进入工具箱后，选择 **1. 解锁 BL 锁**
   - 选择 **解锁方案 4**，按照提示操作
   - 手机进入 **Fastboot 模式** 后，选择对应机型
   - 可能会提示按住按键进入 **9008 模式**，但 **Android 8.1 的 Vivo X9 系列** 无法使用此方法
   - **必须拆机短接或使用 9008 数据线** 进入 **9008 模式**
...
2. 通过 Fastboot **刷入修补后的 Boot 镜像**。
3. 如果 **直接刷入修补后的镜像**，会导致无法进入系统。

### **关于 Root 的一些误区**

之前有传言称 **Vivo X9 系列无法永久 Root，只能通过 `fastboot boot` 进行临时 Root**，但，这是 **错误的**。
事实上，该机型 **可以永久 Root**，但是 **如果不去除分区加密，临时启动也无法进入系统**。

------

### **3.1 获取 Boot 镜像并修补**

我们需要使用 **修改后的 Magisk 23.0** 来修补 Boot 镜像，**Boot 镜像必须来自与你的系统版本匹配的官方固件**。
...
1. 将修补后的 `boot.img` **拖入** **Android Image Kitchen Vivo** 进行解包。
...
由于 `suu` 的特殊性，建议尽量使用 **原生支持 `suu` 的软件**（如爱玩机工具箱等），本文提到的使用 MT 管理器修改的方法仅适用于未加固的软件，因此对于存储空间隔离、黑域等需要 Root 但是又加固的软件无效。故对于存储空间隔离，暂时无解，对于黑域请使用 Shizuku 进行激活。
